...
interface MovieCarouselProps {
  movies?: Movie[]; // Optional, can be passed from parent
}
...
const MovieCarousel: React.FC<MovieCarouselProps> = ({ movies: propMovies }) => {
  ...
  useEffect(() => {
    // If movies are passed as props, use them
    ...
    const fetchMovies = async () => {
      try {
        ...
        // Get latest 5 movies by release date
          ...
          <SwiperSlide key={movie.id}>
            <div className="relative w-full h-full group">
              <img
                ...
                onError={(e) => {
                  e.currentTarget.src = "/images/placeholder-background.jpg"; // Fallback image
                }}
              />
              <div className="absolute inset-0 bg-gradient-to-t from-black/90 to-transparent p-10 flex flex-col justify-end">
                ...
                <div className="mt-4 flex space-x-4 opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition duration-700 delay-300">
                  ...
                  <button 
                    ...
                    onClick={() => navigate(`/movie/${movie.id}#tickets`)}
                  >
                    🎟️ Book Now
                  </button>
                </div>
