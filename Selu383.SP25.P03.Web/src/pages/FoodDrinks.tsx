...
const FoodMenuComponent = () => {
  ...
  return (
    <div className="bg-black text-white min-h-screen py-10 px-6">
      <h1 className="text-4xl font-bold text-center mb-10 text-red-500">🍿 Food & Drinks Menu</h1>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {menu.foodMenuItems.map((item: any) => (
                <div
                  ...
                  className="bg-gray-900 p-6 rounded-xl shadow-lg transition-all hover:scale-105 hover:shadow-red-500/50"
                >
                  <img
                  ...
                  />
                  <h3 className="text-2xl font-semibold text-white">{item.foodItem.name}</h3>
                  <p className="text-gray-400 text-sm">{item.foodItem.description}</p>
                  <p className="text-lg font-bold text-red-400 mt-2">${item.foodItem.price}</p>
                </div>
              ))}
            </div>
          </div>
